...
class Card:
    ...
    def get_sort_key(self) -> tuple:
        ...
        # Type order: NUMBER, SKIP, REVERSE, DRAW_TWO, WILD, WILD_DRAW_FOUR
        type_order = {
            CardType.NUMBER: 0,
            CardType.SKIP: 1,
            CardType.REVERSE: 2, 
...
class UnoGame:
    ...
    def _start_game(self):
        ...
        for i, card in enumerate(self.draw_pile):
            if card.type != CardType.WILD and card.type != CardType.WILD_DRAW_FOUR:
    ...
    def play_multiple_cards(self, player_index: int, card_indices: List[int], chosen_color: Optional[Color] = None) -> Tuple[bool, Optional[str]]:
        ...
        if self._is_playable(first_card):
            if first_card.type in (CardType.WILD, CardType.WILD_DRAW_FOUR) and not chosen_color:
                return False, "Must choose color for wild card"

            # Wild cards can only be played one at a time
            if first_card.type in (CardType.WILD, CardType.WILD_DRAW_FOUR) and len(cards_to_play) > 1:
                return False, "Can only play one wild card at a time"

            # Set the color based on the card type
            if first_card.type == CardType.WILD or first_card.type == CardType.WILD_DRAW_FOUR:
